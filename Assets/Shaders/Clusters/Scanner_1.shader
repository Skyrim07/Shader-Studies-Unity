    ...
    SubShader
    {
         ...
         Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float dis = pow(saturate(distance(i.worldPos, ScannerPos1) / max(ScannerRadius1, 0.01)), _ScannerPower);
                fixed4 sCol = fixed4(Luminance(col),Luminance(col),Luminance(col),1);                
                
                col = (dis<0.95 && dis>0.95 -_EdgeWidth*0.05)?_EdgeColor: lerp(sCol, col, dis);
                if(ScannerMelt1>0)
                      col = lerp(col, col2, ScannerMelt1);
                return col * diffuse;
            }
            ENDCG
        }
