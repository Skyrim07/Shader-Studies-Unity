Shader "Unlit/Microfacet PBR"
{
    ...
    SubShader
    {
        ...
        float3 PBR(float3 pos, float3 normal, float3 albedo, float roughness, float metallic, float ao, float shadow) 
        {
            float3 viewDir = normalize(_WorldSpaceCameraPos - pos);
            float3 lightDir = UnityWorldSpaceLightDir(pos);
            float3 halfDir = normalize(normal + lightDir);
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float3 pbr = PBR(i.worldPos, worldNormal, albedo, roughness, metallic, 1, shadowAtten);
