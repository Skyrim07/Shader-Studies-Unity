		...
		SubShader
	{
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				fixed3 specular = _LightColor0.rgb * _Specular.rgb * pow(max(0, Anisotropic), _Gloss);

				fixed3 diffuse = _LightColor0.rgb * _Diffuse.rgb * saturate(dot(i.worldNormal, lightDir)*0.5+0.5);
